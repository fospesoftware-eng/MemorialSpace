...
const CEMETERY_TYPE_OPTIONS = [
  ...
  },
  {
    ...
    label: "Columbarium",
    icon: Box,
    help: "Stand-alone facility for cremation niches (no in-ground burials).",
  },
] as const;
...
export default function Organizations() {
  ...
  const typeMeta = (val: string | null | undefined) =>
    CEMETERY_TYPE_OPTIONS.find((o) => o.value === val) ??
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Create Cemetery</h1>
          <p className="text-muted-foreground mt-1">
            Start by adding a cemetery with its location, contact details, and
            operating type.
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>New Cemetery</DialogTitle>
              <DialogDescription>
                Add the cemetery profile, location, and contact details before
                creating its map.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm((f) => ({ ...f, cemeteryType: v as CemeteryType }))
                  }
                >
                  <SelectTrigger
                    id="cemetery-type-select"
                    className="mt-1"
                    data-testid="select-cemetery-type"
                  >
                  ...
                  <SelectContent>
                    {CEMETERY_TYPE_OPTIONS.map((opt) => (
                      <SelectItem
                        key={opt.value}
                        value={opt.value}
                        data-testid={`option-cemetery-type-${opt.value}`}
                      >
              ...
              {showColumbariumCheckbox && (
                <div className="flex items-start gap-3 rounded-md border border-border bg-muted/30 p-3">
                  ...
                  <div className="text-sm leading-snug">
                    <Label
                      htmlFor="features-columbarium"
                      className="cursor-pointer font-medium"
                    >
                      Add columbarium module
                    </Label>
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      Enable the dedicated Columbarium module so this church
                      cemetery can also manage cremation niches with photos and
                      a 3D wall view.
                    </p>
                  </div>
                </div>
              ...
              {form.cemeteryType === "columbarium" && (
                <div className="rounded-md border border-primary/40 bg-primary/5 p-3 text-xs text-muted-foreground">
                  Columbarium module is enabled by default for stand-alone
              ...
              <Button
                ...
                disabled={createOrg.isPending || !form.name || !form.slug}
                className="w-full"
                data-testid="button-submit-org"
              >
                {createOrg.isPending ? "Creating..." : "Create Cemetery"}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10">
                      {(() => {
                        const Ic = typeMeta(org.cemeteryType).icon;
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-1 flex-wrap">
                        <span className="inline-flex items-center rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                          {typeMeta(org.cemeteryType).label.replace(
                            " Cemetery",
                            "",
                          )}
                        </span>
