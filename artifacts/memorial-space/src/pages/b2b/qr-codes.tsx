    ...
    <div className="flex items-center justify-center p-4 bg-white rounded-lg w-24 h-24">
      <div className="grid grid-cols-7 gap-0.5 w-full h-full">
        {Array.from({ length: 49 }, (_, i) => {
          const corners = [
            0, 1, 2, 3, 4, 5, 6, 7, 13, 14, 20, 21, 27, 28, 34, 35, 41, 42, 43,
            44, 45, 46, 47, 48,
          ];
          ...
          return (
            <div
              key={i}
              className={`rounded-sm ${filled ? "bg-gray-900" : "bg-transparent"}`}
            />
          );
        })}
      </div>
...
export default function QrCodes() {
  ...
  const { data: qrCodes, isLoading } = useListQrCodes({
  ...
  const handleBulkGenerate = async () => {
    ...
    try {
      ...
      toast({
        title:
          data.created > 0
            ? `Generated ${data.created} new QR codes`
            : "All interment records already have QR codes",
        description: `${data.alreadyCovered + data.created} of ${data.total} interment records are now covered.`,
      });
    } catch (err) {
      toast({
    ...
    createQrCode.mutate(
      {
        data: {
          organizationId: ORG_ID,
          burialId: form.burialId ? Number(form.burialId) : undefined,
          plotId: form.plotId ? Number(form.plotId) : undefined,
          memorialId: form.memorialId ? Number(form.memorialId) : undefined,
        },
      },
      {
        onSuccess: () => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Generate and manage QR codes linking to memorial pages.
          </p>
        </div>
        <div className="flex gap-2">
            ...
            onClick={async () => {
              try {
                ...
                queryClient.invalidateQueries({
                  queryKey: getListQrCodesQueryKey(),
                });
                toast({
                  title:
                    data.updated > 0
                      ? `Issued ${data.updated} edit PINs`
                      : "Every QR already has a PIN",
                  description: `${data.total} QR codes total.`,
                });
              } catch (err) {
                toast({
                  title: "Couldn't issue PINs",
                  description:
          ...
          >
            ...
            {bulking ? "Generating…" : "Generate for all interment records"}
          </Button>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Generate a QR code linking to an interment record, burial
                  spot, or memorial page.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div>
                  <Label>Link to Interment Record (optional)</Label>
                  <select
                    className="w-full mt-1 rounded-md border border-input bg-background px-3 py-2 text-sm"
                    value={form.burialId}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, burialId: e.target.value }))
                    }
                    data-testid="select-burial"
                  >
                ...
                <div>
                  <Label>Link to Burial Spot (optional)</Label>
                  <select
                    className="w-full mt-1 rounded-md border border-input bg-background px-3 py-2 text-sm"
                    value={form.plotId}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, plotId: e.target.value }))
                    }
                ...
                <div>
                  ...
                  <select
                    className="w-full mt-1 rounded-md border border-input bg-background px-3 py-2 text-sm"
                    value={form.memorialId}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, memorialId: e.target.value }))
                    }
                    data-testid="select-memorial"
                  >
                    <option value="">None</option>
                    {memorials?.map((m) => (
              ...
              <CardContent className="pt-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-mono text-sm font-bold tracking-widest text-primary mb-1">
                      {qr.code}
                    </div>
                    ...
                    {qr.editPin ? (
                      ...
                      <div className="text-xs text-amber-600 mb-3">
                        No PIN issued — families can't edit yet.
                      </div>
                    )}
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="flex-1"
                        data-testid={`button-copy-qr-${qr.id}`}
                      >
