...
export default function Bookings() {
  ...
  const { data: bookings, isLoading } = useListBookings({ organizationId: ORG_ID });

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'confirmed': return 'bg-[#40916c]/20 text-[#40916c]';
      case 'pending': return 'bg-[#d4a843]/20 text-[#d4a843]';
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage plot sales and reservations.</p>
        </div>
              ...
              filteredBookings.map((booking) => (
                <TableRow key={booking.id}>
                  <TableCell className="font-medium">
                    ...
                    <div className="text-xs text-muted-foreground font-normal">{booking.customerEmail}</div>
                  </TableCell>
                  <TableCell className="capitalize">{booking.type}</TableCell>
                  <TableCell>{booking.plotId}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className={`capitalize border-none ${getStatusColor(booking.status)}`}>
                      {booking.status}
                    </Badge>
                  </TableCell>
                  <TableCell>{booking.bookingDate ? format(new Date(booking.bookingDate), 'MMM d, yyyy') : '-'}</TableCell>
