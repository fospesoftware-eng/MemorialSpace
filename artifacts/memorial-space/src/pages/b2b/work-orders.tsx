...
export default function WorkOrders() {
  ...
  const { data: comments } = useListWorkOrderComments(
    ...
    { organizationId: ORG_ID },
    {
      query: {
        enabled: !!detail,
        queryKey: getListWorkOrderCommentsQueryKey(detail?.id ?? 0, {
          organizationId: ORG_ID,
        }),
      },
    },
  ...
  const invalidate = () =>
    qc.invalidateQueries({
      queryKey: getListWorkOrdersQueryKey({ organizationId: ORG_ID }),
    });
  const invalidateComments = (id: number) =>
    qc.invalidateQueries({
      queryKey: getListWorkOrderCommentsQueryKey(id, {
        organizationId: ORG_ID,
      }),
    });
  ...
  const addComment = useCreateWorkOrderComment({
    mutation: {
      onSuccess: () => {
        if (detail) invalidateComments(detail.id);
        setCommentText("");
      },
    },
  });
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Cemetery operations: maintenance, cleaning, inspections, and service
            tasks.
          </p>
        </div>
        <Dialog
        ...
        >
          <DialogTrigger asChild>
            <Button className="gap-2" data-testid="button-create-work-order">
            ...
            <div className="grid gap-4 py-2">
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  placeholder="Repair fence at Section B"
      ...
      <div className="grid gap-4 grid-cols-3">
        ...
        <div className="bg-card border rounded-xl p-4 shadow-sm">
          ...
          <div className="text-2xl font-bold mt-1 text-[#40916c]">
            {counts.completed}
          </div>
        </div>
      </div>
              ...
              filtered.map((wo) => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={`capitalize border-none ${STATUS_COLOR[wo.status as Status]}`}
                    >
                      {wo.status.replace("_", " ")}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={`capitalize border-none ${PRIORITY_COLOR[wo.priority as Priority]}`}
                    >
                      {wo.priority}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    {wo.dueDate
                      ? format(parseISO(wo.dueDate), "MMM d, yyyy")
                      : "-"}
                  </TableCell>
            ...
            <>
              ...
              <div className="mt-6 space-y-6">
                <div className="flex items-center gap-2 flex-wrap">
                  <Badge
                    variant="outline"
                    className={`capitalize border-none ${STATUS_COLOR[detail.status as Status]}`}
                  >
                    {detail.status.replace("_", " ")}
                  </Badge>
                  <Badge
                    variant="outline"
                    className={`capitalize border-none ${PRIORITY_COLOR[detail.priority as Priority]}`}
                  >
                    {detail.priority}
                  </Badge>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="mt-1.5">
                      {detail.dueDate
                        ? format(parseISO(detail.dueDate), "MMM d, yyyy")
                        : "—"}
                    </p>
                  </div>
                  <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {detail.status === "open" && (
                    <Button
                      size="sm"
                      className="gap-1.5"
                      onClick={() => transition(detail, "in_progress")}
                  ...
                  {detail.status === "in_progress" && (
                    <Button
                      size="sm"
                      className="gap-1.5"
                      onClick={() => transition(detail, "completed")}
                  ...
                  {(detail.status === "open" ||
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="gap-1.5"
                      onClick={() => transition(detail, "cancelled")}
                    >
                  ...
                  {detail.status !== "open" &&
                    detail.status !== "in_progress" && (
                      <Button
                        ...
                        variant="outline"
                        className="gap-1.5"
                        onClick={() => transition(detail, "open")}
                      >
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="grid gap-1">
                      ...
                      <Input
                        ...
                        step="0.25"
                        min="0"
                        defaultValue={detail.laborHours ?? ""}
                        onBlur={(e) =>
                          e.target.value !== (detail.laborHours ?? "") &&
                          saveCosts(detail, { laborHours: e.target.value })
                        }
                    ...
                    <div className="grid gap-1">
                      ...
                      <Input
                        ...
                        step="0.01"
                        min="0"
                        defaultValue={detail.laborCost ?? ""}
                        onBlur={(e) =>
                          e.target.value !== (detail.laborCost ?? "") &&
                          saveCosts(detail, { laborCost: e.target.value })
                        }
                      />
                    </div>
                    <div className="grid gap-1">
                      ...
                      <Input
                        ...
                        step="0.01"
                        min="0"
                        defaultValue={detail.materialsCost ?? ""}
                        onBlur={(e) =>
                          e.target.value !== (detail.materialsCost ?? "") &&
                          saveCosts(detail, { materialsCost: e.target.value })
                        }
                      />
                    </div>
                  </div>
                ...
                {detail.status === "completed" && (
                  <div className="border rounded-lg p-4 space-y-2">
                    ...
                    <Textarea
                      defaultValue={detail.completionNotes ?? ""}
                      rows={3}
                      onBlur={(e) =>
                        e.target.value !== (detail.completionNotes ?? "") &&
                        saveCosts(detail, { completionNotes: e.target.value })
                      }
