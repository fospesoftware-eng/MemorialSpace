...
A B2C feature on the public memorial page (`/c/:slug/memorial/:code`) letting visitors light candles, offer flower bouquets, and leave written or voice prayers. `memorial_rituals` schema (CASCADE on org + burial) tracks type, variant, visitor name, message, optional base64 audio data URL, duration, and an `expiresAt` driven by per-type lifetimes (24h candles / 7d flowers / 30d prayers). Public API: `GET /api/c/:slug/memorial/:code/rituals` (active list + per-type active/all-time counts) and `POST` (rate-limited 8/10min per ip+code, audio data URLs accepted with `;codecs=` MIME params, prayers require text or audio). Frontend renders an animated candle wall (dual-layer flame with inner core + outer envelope, per-candle CSS-variable seed for out-of-phase flicker, drifting smoke wisps, floating embers, slow wax drip, ambient uplight haze), a flower garden using hand-drawn SVG blooms (cemetery-only taxonomy: white roses, lilies, chrysanthemums, white carnations, forget-me-nots — no emoji clipart, each petal animated open with stagger + gentle sway, drifting petals overlay), and a prayer wall with avatar + animated waveform + audio playback. All animations honour `prefers-reduced-motion`. `VoiceRecorder` uses MediaRecorder up to 60s with strict size cap and full unmount cleanup (mic stream stop + timer clear). Rituals render only when the memorial is unlocked (`!memorial.locked`), keeping PIN-gated memorials fully private. Lives at `lib/db/src/schema/memorialRituals.ts`, `artifacts/api-server/src/routes/memorialRituals.ts`, `artifacts/memorial-space/src/pages/cemetery-site/digital-rituals.{tsx,css}`.

**Marketplace integration.** Rituals are wired into the existing cemetery marketplace so the virtual wall and the real catalogue share data. `cemetery_orders` gained a nullable `burial_id` (FK→burials, ON DELETE SET NULL, indexed) so revenue history is preserved if a burial is removed. The public order POST accepts an optional `memorialCode` which the server resolves through `qr_codes` scoped to the same org → `burialId`; invalid/foreign codes are silently dropped to avoid losing legitimate orders. The rituals GET also returns the cemetery's published catalogue bucketed into `{ candle, flower, prayer }` (max 3 each, featured-first) via keyword classification on category+product name, plus a `realOrderCount` of orders linked to this burial. The memorial page passes `memorialCode` in the marketplace deep-link; the marketplace persists it in the `OrderContext` sessionStorage alongside `for`/`plotRef`; the cart re-reads it on submit. The ritual wall surfaces this as a "X real tributes fulfilled by the cemetery" badge plus a "Send a real one" upsell strip with up to 3 product cards per ritual type that deep-link to product detail with the memorial context attached.

### Stripe Payment Gateway (two-tier)
Two completely separate Stripe accounts power MemorialSpace billing. **Platform** (super admin) — singleton row in `platform_payment_settings` (id=1) with the SaaS company's Stripe keys, used to charge cemetery operators their plan fees. **Per-org** (cemetery owner) — `org_payment_settings` (FK→organizations, unique), each operator brings their own Stripe so customer money settles directly to their bank. Each row stores provider, mode (test|live), enabled, publishable/secret/webhook keys, default currency (3-letter ISO), 22-char statement descriptor, and verification metadata (lastVerifiedAt/lastVerifiedStatus/accountId/accountName/accountEmail/livemode). Server: `paymentGateway.ts` exposes `GET/PUT /admin/payment-gateway` + `/test` and `GET/PUT /orgs/me/payment-gateway` + `/test`. Secrets are masked on every read (`••••••••<last4>`), the UI sends back the masked placeholder unchanged when the user only edits other fields, and the server's `buildPatch` drops any masked round-tripped value. Write access: platform writes restricted to `super_admin`/`billing_admin` (support admins are read-only); org writes restricted to `owner`/`admin`. `/test` calls `stripe.accounts.retrieve()` to verify and persists `ok | invalid_key | mismatched_mode | network_error`. PUT auto-resets verification metadata when any of `mode`/`secretKey`/`publishableKey`/`webhookSecret`/`provider` changes — preserves status when only enabled/currency/descriptor change. Client uses one shared `<PaymentGatewayCard scope="platform"|"org">` component (in `components/payment-gateway-card.tsx`) on both pages: super admin at `/payment-gateway` and cemetery owner at `/settings/payment-gateway`. Stripe SDK pinned to default API version, 8s timeout, 1 network retry. Note: secret keys are stored plaintext in DB (MVP) — wrap with envelope encryption / KMS before production launch.
