...
- **Per-type default shape:** Every `PlotType` carries an optional `defaultShape: "rect" | "circle" | "path"` (defaults to `"rect"` when missing). The Cemetery Setup table exposes a **Shape** column (Select with icon + hint) per row; the Map Maker palette button reads the active type's `defaultShape` and auto-switches the canvas tool via a `shapeToTool()` helper when clicked. Default seeds: PATH/BR=path, WTR/GDN=circle, all other defaults=rect. The user can still pick any tool manually from the toolbar.
